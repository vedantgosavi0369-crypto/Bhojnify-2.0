...
export default function MenuScreen() {
  ...
  const add = () => { if (!dish) { Alert.alert(t('addDish'), t('dishNameMessage')); return; } addMenu({ day, meal, dish, note: note || t('freshlyPrepared') }); setDish(''); setNote(''); };
  return <KeyboardAwareScrollViewCompat style={[styles.screen, { backgroundColor: colors.background }]} contentContainerStyle={styles.content} bottomOffset={24} keyboardShouldPersistTaps="handled"><Header eyebrow={t('ownerTools')} title={t('menuPlanner')} subtitle={t('shapeMeals')} onPress={() => router.push('/(tabs)/profile')} /><View style={[styles.menuHero, { backgroundColor: colors.primary }]}><Ionicons name="calendar-outline" size={26} color={colors.accent} /><View><Text style={[styles.heroTitle, { color: colors.primaryForeground }]}>{t('nextUp')}</Text><Text style={[styles.heroDetail, { color: colors.primaryForeground }]}>{t('planAhead')}</Text></View></View><SectionHeading title={t('publishedMenu')} /><View style={[styles.menuList, { backgroundColor: colors.card, borderColor: colors.border }]}>{menus.map((item, index) => <View key={item.id} style={[styles.menuRow, index < menus.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}><View style={[styles.dayPill, { backgroundColor: colors.secondary }]}><Text style={[styles.dayText, { color: colors.primary }]}>{localizedValue(language, item.day).slice(0, 3).toUpperCase()}</Text></View><View style={styles.menuCopy}><View style={styles.menuTop}><Text style={[styles.mealText, { color: colors.mutedForeground }]}>{localizedValue(language, item.meal)}</Text><Badge label={t('published')} /></View><Text style={[styles.dishText, { color: colors.foreground }]}>{item.dish}</Text><Text style={[styles.noteText, { color: colors.mutedForeground }]}>{item.note}</Text></View></View>)}</View><SectionHeading title={t('addToPlan')} /><View style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}><FormField label={t('day')} value={day} onChangeText={setDay} placeholder={t('enterDay')} /><FormField label={t('mealSlot')} value={meal} onChangeText={(value) => setMeal((value || 'Lunch') as Meal)} placeholder={t('enterMealSlot')} /><FormField label={t('dish')} value={dish} onChangeText={setDish} placeholder={t('enterDishName')} /><FormField label={t('notes')} value={note} onChangeText={setNote} placeholder={t('enterMenuNotes')} /><PrimaryButton label={t('publishMenuItem')} icon="check" onPress={add} /></View></KeyboardAwareScrollViewCompat>;
}
