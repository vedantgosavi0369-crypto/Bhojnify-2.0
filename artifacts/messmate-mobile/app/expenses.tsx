...
export default function ExpensesScreen() {
  ...
  const add = () => { if (!amount || !note) { Alert.alert('Complete expense', 'Add an amount and a note.'); return; } addExpense({ category, amount: Number(amount), note, date: new Date().toISOString().slice(0, 10) }); setAmount(''); setNote(''); };
  return <KeyboardAwareScrollViewCompat style={[styles.screen, { backgroundColor: colors.background }]} contentContainerStyle={styles.content} bottomOffset={24} keyboardShouldPersistTaps="handled"><Header eyebrow="Owner tools" title="Expenses" subtitle="Capture every cost for a cleaner monthly P&L." onPress={() => router.push('/(tabs)/profile')} /><View style={[styles.totalCard, { backgroundColor: colors.foreground }]}><View><Text style={[styles.totalLabel, { color: colors.card }]}>SEPTEMBER SPEND</Text><Text style={[styles.totalValue, { color: colors.card }]}>₹{expenses.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}</Text></View><Ionicons name="trending-down-outline" size={30} color={colors.accent} /></View><SectionHeading title="Recent expenses" /><View style={[styles.list, { backgroundColor: colors.card, borderColor: colors.border }]}>{expenses.map((expense, index) => <View key={expense.id} style={[styles.expenseRow, index < expenses.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}><View style={[styles.expenseIcon, { backgroundColor: colors.secondary }]}><Ionicons name={expense.category === 'Gas' ? 'flame-outline' : expense.category === 'Maintenance' ? 'construct-outline' : 'cart-outline'} size={18} color={colors.primary} /></View><View style={styles.expenseCopy}><Text style={[styles.expenseTitle, { color: colors.foreground }]}>{expense.category}</Text><Text style={[styles.expenseNote, { color: colors.mutedForeground }]}>{expense.note}</Text></View><Text style={[styles.expenseAmount, { color: colors.foreground }]}>₹{expense.amount.toLocaleString()}</Text></View>)}</View><SectionHeading title="Log an expense" /><View style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}><FormField label="Category" value={category} onChangeText={setCategory} placeholder="Gas, transport, maintenance" /><FormField label="Amount" value={amount} onChangeText={setAmount} placeholder="0" keyboardType="numeric" /><FormField label="Note" value={note} onChangeText={setNote} placeholder="What was this for?" /><PrimaryButton label="Save expense" icon="plus" onPress={add} /></View></KeyboardAwareScrollViewCompat>;
}
