...
export default function FeedbackScreen() {
  ...
  return <Screen>
    <Header eyebrow={t('ownerTools')} title={t('feedbackTitle')} subtitle={t('feedbackSubtitle')} onPress={() => router.push('/(tabs)/admin')} />
    <View style={[styles.summary, { backgroundColor: colors.primary }]}><Ionicons name="chatbubble-ellipses-outline" size={24} color={colors.accent} /><View style={styles.summaryCopy}><Text style={[styles.summaryTitle, { color: colors.primaryForeground }]}>{t('notesRecorded', { count: feedback.length, plural: feedback.length === 1 ? '' : 's' })}</Text><Text style={[styles.summaryDetail, { color: colors.primaryForeground }]}>{t('improveMenu')}</Text></View></View>
    <SectionHeading title={t('recentNotes')} />
    <View style={[styles.list, { backgroundColor: colors.card, borderColor: colors.border }]}>
      {feedback.length ? feedback.map((item, index) => <View key={item.id} style={[styles.row, index < feedback.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.note, { color: colors.foreground }]}>{item.note || t('rated', { dish: item.dish })}</Text>
        <Text style={[styles.meta, { color: colors.mutedForeground }]}>{item.dish} · {item.date}</Text>
      </View>) : <Text style={[styles.empty, { color: colors.mutedForeground }]}>{t('noFeedback')}</Text>}
    </View>
  </Screen>;
}
